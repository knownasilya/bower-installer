...
function installFile(f, pakcfg, paths, dep, silent, callback) {
    ...
    f_name = f_s.indexOf(basePath) === 0 ? f_s : pathLib.join(basePath, f_s);
    ...
    if (paths.all) {
        ...
        path = pathLib.join(utils.parsePath(paths.all, pakcfg), key, subDir);
    ...
    mkdirp.sync(pathLib.normalize(pathLib.join(basePath, path)), 0755);

    var name = f_name_new ? f_name_new : pathLib.basename(f_name);
    var f_path = pathLib.join(basePath, path, utils.parsePath(name, pakcfg));
   ...
   if (fs.lstatSync(f_name).isDirectory()) {
        ...
        var packagejson = pathLib.join(f_name, 'package.json');
        ...
        try {
            ...
            var mainpath = pathLib.join(f_name, filedata.main);
            ...
            if (fs.lstatSync(mainpath).isFile()) {
                ...
                f_path = pathLib.join(basePath, path, filedata.main);
            }
...
exports.removeComponentsDir = function(callback) {
    utils.deleteFolderRecursive(pathLib.join(basePath, 'bower_components'));
    callback();
}
